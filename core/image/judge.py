...
from config import (
    IMAGE_AI_THRESHOLD, JUDGE_MODE, GEMINI_API_KEY, JUDGE_LLM_MODEL,
    GROQ_API_KEY, GROQ_MODEL, JUDGE_LLM_PROVIDER,
)
...
class Judge:
    ...
    def _weighted_ensemble(
        ...
        """Weighted average with conflict detection and override rules."""
        ...
        # --- Override rules ---
        # If any high-confidence analyzer scores very high, pull score up
        max_confident_score = 0.0
        for r in valid_results:
            if r.confidence >= 0.7 and r.signal_score > max_confident_score:
                max_confident_score = r.signal_score

        # Strong AI signal from a confident analyzer
        if max_confident_score > 0.85:
            final_score = max(avg_score, max_confident_score * 0.7 + avg_score * 0.3)
        # Strong human signal
        elif max_confident_score < 0.15 and avg_score < 0.3:
            final_score = min(avg_score, 0.15)
        else:
        ...
        # --- Explanation ---
        explanation = self._build_explanation(valid_results, final_score, verdict_str, has_conflict)
...
{{
  ...
  "generator_guess": "Likely DALL-E/Midjourney/Stable Diffusion/etc" or null
    ...
    def _build_explanation(
        ...
        strong_human = [r for r in results if r.signal_score < 0.4 and r.confidence >= 0.4]

        parts = []
        if verdict == "AI Generated":
            ai_reasons = [r.reasoning for r in strong_ai[:3]]
            parts.append(f"Image shows strong indicators of AI generation ({score:.0%} probability).")
            if ai_reasons:
                parts.append("Key signals: " + "; ".join(ai_reasons) + ".")
        elif verdict == "Human Created":
            human_reasons = [r.reasoning for r in strong_human[:3]]
            parts.append(f"Image appears to be human-created ({1 - score:.0%} probability).")
            if human_reasons:
                parts.append("Supporting evidence: " + "; ".join(human_reasons) + ".")
        else:
            parts.append("Analysis is inconclusive — analyzers show conflicting signals.")
            if strong_ai:
        ...
        if has_conflict:
            parts.append("Note: significant disagreement between analyzers reduces confidence.")
